        ...
        child: Column(
          ...
          children: [
            ...
            _infoCard(children: const [
              ...
              _Bullet(
                  'Costo Mantenimiento (H): costo anual por unidad almacenada.'),
              _Bullet(
                  'Costo por Faltante (opcional): costo por unidad no atendida.'),
              _Bullet('Costo Unitario: precio por unidad del artículo.'),
              _Bullet('Espacio por Unidad (m²): área ocupada por cada unidad.'),
              _Bullet(
                  'Desviación Estándar Diaria (σd): variabilidad de la demanda diaria.'),
              _Bullet(
                  'Punto de Reorden (R): nivel de inventario a partir del cual se emite un pedido.'),
              _Bullet('Tamaño de Lote (Q): cantidad de unidades por pedido.'),
            ]),
            const SizedBox(height: 24),
            _sectionTitle('Fórmulas usadas (Modelo EOQ y Punto de Reorden)'),
            _infoCard(children: [
              const _Formula('Tamaño de lote óptimo (EOQ):',
                  r"Q^* = \sqrt{\frac{2\,D\,S}{H}}"),
              const SizedBox(height: 8),
              const _Formula(
                  'Demanda diaria promedio (d):', r"d = \frac{D}{365}"),
              const _Formula('Desviación durante el lead time (σL):',
                  r"\sigma_L = \sigma_d\,\sqrt{L}"),
              const _Formula(
                  'Punto de reorden (R):', r"R = d\,L + z\,\sigma_L"),
              const SizedBox(height: 8),
              const _Plain('Donde:'),
              ...
              const _Bullet(
                  'L: lead time (días) configurado en Restricciones.'),
              const _Bullet('σd: desviación estándar diaria.'),
              const _Bullet(
                  'z: factor de servicio (derivado del nivel de servicio objetivo).'),
            ]),
            const SizedBox(height: 24),
            _sectionTitle('Componentes de costo (política Q,R con backorders)'),
            _infoCard(children: const [
              _Formula('Inventario promedio anual:',
                  r"\bar{I} = \frac{Q}{2} + \max(0, R - \mu_L) - E[BO]"),
              SizedBox(height: 8),
              _Formula(
                  'Costo de mantenimiento anual:', r"C_h = H \cdot \bar{I}"),
              _Formula('Costo de faltante anual:',
                  r"C_p = p \cdot \frac{D}{Q} \cdot E[BO]"),
              ...
              _Plain('Notas:'),
              _Bullet(
                  'μL: demanda esperada durante el lead time (μL = D · L/365).'),
              _Bullet(
                  'E[BO]: backorders esperados (usando la función de pérdida normal estándar).'),
            ]),
            const SizedBox(height: 24),
        ...
        Container(
          ...
          child: Math.tex(
            ...
            textStyle: GoogleFonts.poppins(
                fontSize: 20, color: MDSJColors.textPrimary),
          ),
        ),
      ],
    );
